...
export default async function generateAllGames({
  chat_history,
  moduleId,
  levelId,
}: any) {
...
Do not include text outside the JSON.`,
    ...
    try {
      ...
      console.error("[generateAllGames] JSON.parse failed on response body:", bodyText);
      const match = bodyText.match(/\{[\s\S]*\}|\[[\s\S]*\]/);
      if (match) {
        try {
          parsed = JSON.parse(match[0]);
        } catch (matchErr) {
          console.error("[generateAllGames] Regex JSON parse failed");
        }
      }
    }

    if (!parsed) {
      return { error: "AI service returned non-JSON text. Please try again." };
    }
