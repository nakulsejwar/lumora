...
export default async function generateQuiz(content: string, length: number) {
  try {
    const aiResponse = await strict_output(
      ...
      `You are to generate a a set of ${length} multiple-choice questions (MCQs) with answers for the following html content ${content}. Provide proper reasoning for each correct answer. Please ignore the html tags`,
      {
        questions: [
          {
            question: "question",
