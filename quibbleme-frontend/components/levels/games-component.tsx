...
function GamesComponent({ levels }: { levels: any }) {
  ...
  return (
    <>
      <AnimatePresence mode="wait">
        <motion.div
          className={`relative z-10 flex py-5 md:p-5 min-h-screen h-full flex-col items-center justify-start bg-gameSwipe.neutral text-gray-700 text-center`}
          ...
          <div className="">
            {combinedQueries.data && combinedQueries.data.length ? (
              <div className="grid md:grid-cols-2 lg:grid-cols-3 lg:place-items-center lg:place-content-center gap-10 max-w-7xl mx-auto ">
                {combinedQueries.data.map((game: any) => (
                  <div
                    key={game.gameId}
                    className={cn(
                      "relative bg-white flex flex-col flex-1 border border-blue-200 shadow-lg  h-[320px]  w-full max-w-[310px] md:max-w-[280px]  rounded-lg ",
                      gamesPlayed
                        ? gamesPlayed.some(
